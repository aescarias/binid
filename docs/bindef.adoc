...
{ id: ListChunk, type: struct, endian: "little",
  fields: [        
    ...
    { type: var,     id: _end, value: file.pos + length },
...
The purpose of `file.pos` is explained in <<Builtins>>.
...
----
{
  type: enum[uint8],
  fields: [
    { id: RESERVED, value: {from: 0, to: 127}, doc: "Reserved" },
    { id: APP_1,    value: 128,                doc: "Application 1"}
  ]
}
----
...
Builtins are globally available methods or variables that contain information about the file context as well as utilities that operate on top of that context. Currently, only one builtin is defined: `file.pos`.

The `file` variable is a mapping containing as of now a single key `pos` which defines the current byte offset in the file.
